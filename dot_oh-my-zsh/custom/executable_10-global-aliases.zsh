...
# git
alias cpdb='cp_dependabot_branches'
...
function cp_dependabot_branches() {
  for branch in $(git branch -r | grep dependabot/npm_and_yarn); do
    git log $branch --pretty=format:"%H" | while read commit; do
      git cherry-pick -m 1 $commit || {
        echo "Merge conflict detected. Resolve the conflict and then run 'git cherry-pick --continue' to proceed."
        return 1
      }
    done
  done
